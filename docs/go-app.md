...
> [!TIP]
> In a standard development scenario, it may make more sense for the Go App and Helm Charts to live in a `src/` directory and is built at runtime. However, we intentionally decoupled the application and its Helm charts into this structure. This allows the Docker container and Helm charts to be published completely independently of the UDS package.
> By doing this, external engineers can pull and use our container and Helm chart for their own purposes. Ialso provides a realistic demonstration of how a UDS package pulls external artifacts via the `common/zarf.yaml` and `.zarf.yaml` files.
...
3. Commit and push your changes. This will trigger the `.github/workflows/release-container-and-charts.yaml` GitHub Action, which builds and publishes the new container image and Helm chart directly to GHCR. This workflow is set to only run on PRs to the following:
```yaml
    paths:
